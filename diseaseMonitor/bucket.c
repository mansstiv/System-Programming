...
void insert_patient_to_bucket(struct bucket * bucket, patientRecord * patient, int bucketSize , int flag)
{
	...
	while(1)
	{
		...
		for (int i = 0; i < tempBucket->numOfEntries; ++i)
		{
			...
			// diseaseHashtable case
			if(flag == 0)
			{
				// Found appropriate bucketEntry so insert to tree
				if (strcmp(tempBucketEntry.key, patient->diseaseID) == 0) 
				{
					tempRBTnode = new_node(patient);
					RBTinsert(tempBucketEntry.tree, tempRBTnode);
					found = 1;
					break;
				}
			}
			else //countryHashtable case
			{
				// Found appropriate bucketEntry so insert to tree
				if (strcmp(tempBucketEntry.key, patient->country) == 0) 
				{
					tempRBTnode = new_node(patient);
					RBTinsert(tempBucketEntry.tree, tempRBTnode);
					found = 1;
	...
	if (found == 0)
	{
		...
		{
			initialize_bucket_entry(&tempBucketEntry, patient->country);
			tempRBTnode = new_node(patient);
			RBTinsert(tempBucketEntry.tree, tempRBTnode);
			tempBucket->numOfEntries++;
			memmove(tempBucket->bytes + (tempBucket->numOfEntries - 1) * sizeof(struct bucketEntry), &tempBucketEntry, sizeof(struct bucketEntry));

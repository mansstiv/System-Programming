...
int main(int argc, char *argv[])
{
    ...
    int numWorkers = -1, bufferSize = -1, status = 0, temp = 0;
    char *input_dir = NULL;
    ...
    if (numWorkers < 0 || bufferSize < 0 ||input_dir == NULL)
    {
    ...
    // Reading and printing statistics being sent by workers
    temp_str = NULL;
    read_message_with_select(numWorkers, fd, bufferSize, &temp_str);
    printf("%s", temp_str);
    free(temp_str);
